...
public class Simulator {
	...
	public static SimulatorOutput simulate(SimulatorInput inputs) {
		...
		for (double x : temperatureOutput) {
			simulatorOutput.setForecastTemp((Math.round(x * 10D)/10D));
		}
		double[] pressure = weatherforecast.getForecastData(pressureArray);
		for (double x : pressure) {
			simulatorOutput.setForecastPressure((Math.round(x * 10D)/10D));
		}
		double[] humidity = weatherforecast.getForecastData(humidityArray);
		for (double x : humidity) {
			simulatorOutput.setForecastHumidity((Math.round(x * 10D)/10D));
		}
	...
	// Outputs a string taking latitude , longitude and elevation from the cordMap which was
